# Intro to Artificial Intelligence - Final Project 🤖

**Lecturer:** Dr. Akiva Bruno Melka  
**Teaching Assistant:** Mr. Regev Yehezkel Imra  
**Students:** Yuval Nerson & Natan Lerner

## Project Overview
This repository contains the implementation of the final project for the Introduction to Artificial Intelligence course. The project focuses on supervised learning through distinct tasks, demonstrating modeling choices, evaluation methodology, and practical implementation.

## 📁 Repository Structure
* [Part I: Regression Task - Auto MPG](./Part_I_Regression_Task.ipynb) – Predicts fuel efficiency (MPG) using Linear, Polynomial, and KNN regression models, including a detailed Exploratory Data Analysis (EDA).
* [Part II: Classical Classification Task - CIFAR-10](./Part_II_Classical_Classification_Task.ipynb) – Multi-class classification on the CIFAR-10 dataset using Logistic Regression, SVM, and KNN models.
* [Part III: Neural Network Classification (PyTorch)](./Part_III_Neural_Network_Classification_(PyTorch).ipynb) – A PyTorch-based implementation of a neural network classifier, including hyperparameter optimization and architectural design.
* [Part IV: Bonus Task (Regression DNN)](./Part_IV_Bonus_Task_Neural_Network_Regression_with_Py_Torch.ipynb) – Neural Network Regression with PyTorch, adapting the classification model to solve the regression problem from Part I.

## Reproducibility
All numerical results reported in the accompanying academic report (Overleaf), including the final Test RMSE of 3.0859 and optimization curves, exactly match the outputs generated by the code in this repository.
